...
from ..sensitivity_computation.compute_sensitivity_based_on_data import compute_variance_sensitivity
...
def variance(array,
    ...
    :param bounds:   tuple type, optional;             Bounds of the values of the array, which is in the form of
                                                       (min, max).
    ...
    # Add LaplaceTruncated noise to the actual variance
    # -------------------------------------------------------------------------------------------------------------
    _func      = np.nanvar if nan else np.var
    actual_var = _func(array,
                       axis     = axis,
                       dtype    = dtype,
                       keepdims = keepdims)
    ...
    if isinstance(privacy_notion, EpsilonDp):
        dp_mech = LaplaceTruncated(epsilon      = epsilon,
                                   delta        = 0,
                                   sensitivity  = compute_variance_sensitivity(lower=lower, upper=upper, array_size=array.size),
                                   lower        = 0,
